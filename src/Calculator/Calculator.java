...
public class Calculator {
    ...
    private Integer multiply(Integer n1, Integer n2) {
        return (isZero.test(n1) || isZero.test(n2)) ? 0 : isPositive.test(n2) ?
                sum.calculate(n1, multiply(n1, subtract.calculate(n2, 1))) :
                subtract.calculate(multiply(n1, sum.calculate(n2, 1)), n1);
    }
    ...
    private Integer divide(Integer n1, Integer n2) {
        ...
        int signs = multiply(Integer.signum(n1),Integer.signum(n2));
        return isZero.test(subtract.calculate(Math.abs(n1), Math.abs(n2))) ?
                signs :
                multiply(signs, sum.calculate(1, divide(subtract.calculate(Math.abs(n1), Math.abs(n2)), Math.abs(n2))));
    }
